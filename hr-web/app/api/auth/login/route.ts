...
import { LoginInput } from '@/lib/api/contracts/auth';
import { ok, fail, zodFields } from '@/lib/api/respond';
import { Errors } from '@/lib/api/errors';

export async function POST(req: NextRequest) {
  try {
    const parsed = LoginInput.safeParse(await req.json().catch(() => ({})));
    if (!parsed.success) throw Errors.validation(zodFields(parsed.error));
    const meta = {
      ip: req.headers.get('x-forwarded-for') ?? undefined,
      userAgent: req.headers.get('user-agent') ?? undefined,
    };
    const result = await login(parsed.data.username, parsed.data.password, meta);
