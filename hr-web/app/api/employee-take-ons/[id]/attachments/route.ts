...
/** List a take-on's onboarding-document attachments. */
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db
    .select()
    .from(attachments)
    .where(and(eq(attachments.entityType, 'take_on'), eq(attachments.entityId, id), isNull(attachments.deletedAt)))
    .orderBy(desc(attachments.createdAt));
  return NextResponse.json({ value: { items: rows, total: rows.length } });
}

/** Upload an onboarding document for a take-on (multipart: file, category). */
